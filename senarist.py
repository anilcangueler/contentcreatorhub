...
def get_model():
    try:
        ...
        safe_models = [m for m in models if "exp" not in m and "vision" not in m]
...
def show_history_item(item):
    ...
    with col_d1:
        # Green button for Close (using CSS styling hack or just standard for now, user asked for green)
        # Streamlit buttons are hard to color individually without custom components or heavy CSS.
        # We will use a standard button but maybe add an emoji to signify 'Good/Close'.
        # Actually, let's try to inject a style for this specific button if possible, but for now standard.
        if st.button("✅ Kapat", key="close_dialog"):
            st.rerun()
            
    with col_d2:
        # Red button for Restore
        ...
        if not api_key: 
            ...
            with st.status("🚀 Senaryo hazırlanıyor...", expanded=True) as status:
                ...
                if st.session_state.get('style_ref'):
                    ...
                    4. Üslup: {character} bir ton kullan.
                ...
                save_to_history(res.text, "İlk Taslak")
                status.update(label="✅ Senaryo Tamamlandı!", state="complete", expanded=False)
                st.rerun()

# 2. EDİTÖR EKRANI
else:
    col_editor, col_ai = st.columns([7, 3])
    
    with col_editor:
        # --- GEÇMİŞ BUTONU ---
        c1, c2 = st.columns([8, 2])
        with c1: st.subheader("Konuşma Metni")
        with c2:
            if st.button("📜 Geçmiş"):
                st.session_state['show_history'] = not st.session_state['show_history']
        
        if st.session_state['show_history']:
            with st.expander("Versiyon Geçmişi", expanded=True):
                if not st.session_state['history']:
                    st.info("Henüz geçmiş yok.")
                else:
                    for i, item in enumerate(reversed(st.session_state['history'])):
                        # Use a callback or just check the button click to open dialog
                        if st.button(f"🕒 {item['time']} - {item['label']}", key=f"hist_{i}"):
                            show_history_item(item)

        # --- QUICK REVISE TOOLBAR ---
        st.markdown("##### ⚡ Hızlı İşlemler")
        quick_actions = [
            "Daha Kısa", "Daha Uzun", "Daha Resmi", "Daha Samimi", 
            "Daha Komik", "Daha Ciddi", "Daha Akıcı", "Daha Heyecanlı", 
            "Sadeleştir", "Vurgulu Yap"
        ]
        
        # Create rows of buttons
        cols = st.columns(5)
        for i, action in enumerate(quick_actions):
            if cols[i % 5].button(action, key=f"quick_{i}", use_container_width=True):
                if not api_key: st.error("API yok.")
                    ...
                    with st.status(f"⚡ '{action}' uygulanıyor...", expanded=True) as status:
                        ...
                        # CRITICAL FIX: Get live text from widget key 'main_editor' if available, else fallback
                        ...
                        """
                        res = model.generate_content(revize_prompt)
                        st.session_state['script_content'] = res.text
                        st.session_state['editor_key'] = st.session_state.get('editor_key', 0) + 1  # Force widget refresh
                        save_to_history(res.text, f"Hızlı: {action}")
        ...
        if st.button("✨ Revize Et", type="primary"):
            if not api_key: st.error("API yok.")
                ...
                with st.status("✨ Revize ediliyor...", expanded=True) as status:
                    ...
                    st.session_state['script_content'] = res.text
                    st.session_state['editor_key'] = st.session_state.get('editor_key', 0) + 1  # Force widget refresh
                    save_to_history(res.text, f"Revize: {st.session_state.revize_input[:20]}...")
